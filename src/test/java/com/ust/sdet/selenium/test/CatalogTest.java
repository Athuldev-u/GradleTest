...
public class CatalogTest {
    ...
    @Test
    @DisplayName("Selecting electronics from category show product count")
    void selectProductCategoryShowcount()
    {
            WebElement category=wait.until(ExpectedConditions.visibilityOfElementLocated(PRODUCT_CARD));
            new Select(driver.findElement(CATEGORY_FILTER)).selectByVisibleText("Electronics");
            wait.until(ExpectedConditions.stalenessOf(category));
            wait.until(ExpectedConditions.numberOfElementsToBeMoreThan(PRODUCT_CARD,0));
            List<WebElement> prodcount =driver.findElements(PRODUCT_CARD);
            int count=prodcount.size();
            String display_count=driver.findElement(RESULT_COUNT).getText();
            assertEquals("Showing 2 products",display_count,"card count must be equal to 2 ");
            assertEquals(2,count,"products count is not 2");
    }
    ...
    private void search(String query,String expectedTesultCount){
        ...
        wait.until(ExpectedConditions.textToBePresentInElementLocated(RESULT_COUNT,expectedTesultCount));
    }
}
